  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      if (data) {
        ...
        setExistingFile(data.boq_file_path)
      }
      setLoading(false)
    }
    fetchProject()
    ...
    <div className="p-8 max-w-xl mx-auto animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
          {['project_name', 'beneficiary_name', 'installer_name'].map(field => (
            <div key={field}>
              <label className="block text-sm font-medium text-gray-700 mb-1.5 capitalize">
                {field.replace('_', ' ')} <span className="text-red-400">*</span>
              </label>
              <input
                type="text"
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          {newFile ? (
            <div className="flex items-center gap-3 p-3 bg-brand-50 rounded-xl">
              ...
              <span className="text-sm flex-1 truncate">{newFile.name}</span>
            ...
            <div
              onClick={() => fileInputRef.current?.click()}
              className="border-2 border-dashed border-gray-200 rounded-xl p-5 text-center cursor-pointer hover:border-brand-300 hover:bg-brand-50/50 transition-all"
            >
              <Upload className="w-5 h-5 text-gray-300 mx-auto mb-2" />
              <p className="text-sm text-gray-500">Upload BOQ <span className="text-brand-600">browse</span></p>
            </div>
          )}
          <input
            ref={fileInputRef}
        ...
        <button
          type="submit"
          disabled={saving}
          className="w-full py-3 bg-brand-600 hover:bg-brand-700 disabled:opacity-60 text-white font-medium rounded-xl transition-all active:scale-[0.98]"
        >
          {saving ? (
