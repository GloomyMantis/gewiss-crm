...
export default function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      const { data } = await supabase.from('projects').select('*').eq('id', params.id).eq('designer_id', user.id).single()
        ...
        {boqUrl && (
          <div className="p-5">
            ...
            <a href={boqUrl} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-3 py-2 bg-orange-50 text-orange-700 text-sm font-medium rounded-xl hover:bg-orange-100 transition-colors">
